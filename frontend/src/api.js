...
export const apiWithPrefix = axios.create({
  baseURL: process.env.NODE_ENV === 'production'
    ? 'https://backend-qw27yrwzw-diogusvinceres-projects.vercel.app/api/auth'
    : 'http://localhost:5000/api/auth', // URL do backend em desenvolvimento
});

// Instância para rotas sem '/api'
export const apiWithoutPrefix = axios.create({
  baseURL: process.env.NODE_ENV === 'production'
    ? 'https://backend-qw27yrwzw-diogusvinceres-projects.vercel.app/books'
    : 'http://localhost:5000/books', // URL do backend em desenvolvimento
});
